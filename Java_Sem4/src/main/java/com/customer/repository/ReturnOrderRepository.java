...
public class ReturnOrderRepository {
	...
	public List<ReturnOrderDetail> findReturnOrderDetailsByReturnOrderId(int returnOrderId) {
		try {
			String sql = String.format("""
					...
					WHERE rod.%s = ?
					""", Views.COL_ORDER_DETAIL_QUANTITY, Views.COL_ORDER_DETAIL_PRICE, Views.COL_PRODUCT_NAME,
					Views.COL_PRODUCT_IMG, Views.TBL_RETURN_ORDER_DETAIL, Views.TBL_ORDER_DETAIL,
					Views.COL_RETURN_DETAIL_ORDER_DETAIL_ID, Views.COL_ORDER_DETAIL_ID, Views.TBL_PRODUCT,
					Views.COL_ORDER_DETAIL_PRODUCT_ID, Views.COL_PRODUCT_ID, Views.COL_RETURN_DETAIL_RETURN_ID);
	...
	public boolean updateStatusAndMessage(int returnOrderId, String status, String message, int employeeId) {
		try {
			...
			int rowsAffected = db.update(sql, status, message, employeeId, // Thêm employeeId vào
					returnOrderId);
			return rowsAffected > 0;
		} catch (DataAccessException e) {
