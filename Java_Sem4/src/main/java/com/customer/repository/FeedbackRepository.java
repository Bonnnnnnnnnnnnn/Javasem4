...
import com.mapper.Feedback_mapper;
...
public class FeedbackRepository {
    ...
    public boolean addFeedback(Feedback feedback) {
        try {
            // 1. Thêm feedback
            String sqlFeedback = String.format(
                "INSERT INTO %s (%s, %s, %s, %s, %s, %s) VALUES (?, ?, ?, ?, ?, ?)",
                Views.TBL_FEEDBACK,
                Views.COL_FEEDBACK_PROID,
                Views.COL_FEEDBACK_ORDID,
                Views.COL_FEEDBACK_RATE,
                Views.COL_FEEDBACK_COMMENT,
                Views.COL_FEEDBACK_CREATEDATE,
                Views.COL_FEEDBACK_STATUS
            );

            LocalDateTime creationDateTime = LocalDateTime.now();
            Timestamp creationTimestamp = Timestamp.valueOf(creationDateTime);

            int feedbackRows = db.update(
                sqlFeedback,
                feedback.getProduct_Id(),
                feedback.getOrderDetail_Id(),
                feedback.getRating(),
                feedback.getComment(),
                creationTimestamp,
                true
            ...
            if (feedbackRows > 0) {
                String sqlUpdateOrderDetail = String.format(
                    "UPDATE %s SET Status = 'Reviewed' WHERE %s = ?",
    ...
    public double calculateAverageRating(int productId) {
        try {
            String sql = String.format(
                "SELECT AVG(CAST(%s AS FLOAT)) as average_rating " +
                "FROM %s " +
                "WHERE %s = ?",
                Views.COL_FEEDBACK_RATE,
                Views.TBL_FEEDBACK,
    ...
    public List<Feedback> getProductFeedbacks(int productId) {
        try {
            String sql = String.format(
                "SELECT f.*, " +
                "CONCAT(c.%s, ' ', c.%s) as CustomerName " + 
                "FROM %s f " +
                "INNER JOIN %s od ON f.%s = od.%s " +
                "INNER JOIN [%s] o ON od.%s = o.%s " +
                "INNER JOIN %s c ON o.%s = c.%s " +
                "WHERE f.%s = ? " +
                "ORDER BY f.%s DESC",
                Views.COL_CUSTOMER_FIRST_NAME, Views.COL_CUSTOMER_LAST_NAME,
                Views.TBL_FEEDBACK,
                Views.TBL_ORDER_DETAIL, Views.COL_FEEDBACK_ORDID, Views.COL_ORDER_DETAIL_ID,
                Views.TBL_ORDER, Views.COL_ORDER_DETAIL_ORDER_ID, Views.COL_ORDER_ID,
                Views.TBL_CUSTOMER, Views.COL_ORDER_CUSTOMER_ID, Views.COL_CUSTOMER_ID,
                Views.COL_FEEDBACK_PROID,
                Views.COL_FEEDBACK_CREATEDATE
            );
