...
public class SalaryRepository {
    ...
    private static final String SELECT_ALL_TYPES = 
        String.format("SELECT * FROM %s", Views.TBL_SALARY_TYPES);
    ...
    // Salary History
    private static final String SELECT_HISTORY_BY_EMPLOYEE = 
        String.format("SELECT * FROM %s WHERE %s = ?", 
    ...
    public List<EmployeeSalaryHistory> findHistoryByEmployee(int employeeId) {
        return jdbcTemplate.query(SELECT_HISTORY_BY_EMPLOYEE, 
            new EmployeeSalaryHistoryMapper(), employeeId);
    }
    
    public void saveHistory(EmployeeSalaryHistory history) {
        jdbcTemplate.update(INSERT_HISTORY,
            history.getEmployeeId(),
            history.getSalaryTypeId(),
            history.getAmount(),
